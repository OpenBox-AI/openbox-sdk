...
GOVERNANCE_CHECKLIST_LIMITATIONS = ${py(matrix.governanceChecklistLimitations)}
GOVERNANCE_CHECKLIST_ROWS = ${py(matrix.governanceChecklistRows)}
GOVERNANCE_CHECKLIST_SCORE = ${py(matrix.governanceChecklistScore)}
REFERENCE_PROVIDER_PARITY_CLOSURES = ${py(matrix.referenceProviderParityClosures)}
REFERENCE_PROVIDER_RUNTIME_AUDIT = ${py(matrix.referenceProviderRuntimeAudit)}
PROVIDER_EVENT_CATALOG = ${py(matrix.eventCatalog)}
PROVIDER_PLUGIN_COMPONENTS = ${py(matrix.pluginComponents)}
CLAUDE_CODE_GOVERNANCE_AUDIT_SURFACE = ${py(matrix.claudeCodeGovernanceAuditSurface)}
...
BEHAVIOR_RULE_CAPABILITY_GUARDS = ${py(matrix.behaviorRuleCapabilityGuards)}
GUARDRAILS_HUB_RECORDING_SURFACE = ${py(matrix.guardrailsHubRecordingSurface)}
