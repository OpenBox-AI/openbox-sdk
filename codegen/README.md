...
| `ts/client` | `specs/typespec/backend/main.tsp` | Types via `openapi-typescript` → `ts/types/src/generated/backend.ts`; method coverage via `ts/client/src/generated/endpoint-manifest.ts` (`BACKEND_ENDPOINT_MANIFEST`, 136 entries). | Every method MUST use `Backend.paths['/...']['<verb>']` row types from `openbox-sdk/types`. `tests/unit/endpoint-coverage.test.ts` walks the manifest and fails CI if any entry lacks a `this.<verb>(<path>, ...)` call on the wrapper. Allowlist for browser-driven OAuth / onboarding flows lives next to the test. |
| `ts/core-client` | `specs/typespec/core/main.tsp` (will be replaced by the proposal in `the-core-service` once merged) | Same `openapi-typescript` flow → `ts/types/src/generated/core.ts`; `CORE_ENDPOINT_MANIFEST` (4 entries). | Same rule as `ts/client`, plus: verdict arms come from `specs/typespec/govern/main.tsp`'s `Verdict` enum. |
| `ts/types` | OpenAPI specs (both) | `backend.ts` + `core.ts` (entirely generated). | No hand-written code in this package. |

Every package now has automatic enforcement. Both per-service allowlists in `tests/unit/endpoint-coverage.test.ts` are empty - every endpoint declared in the spec has a wrapper method. The Keycloak browser redirect (the only flow the SDK doesn't drive directly) is a navigation, not an HTTP call, and never appears in the manifests.
