...
| `ts/core-client` | `specs/typespec/core/main.tsp` (will be replaced by the proposal in `openbox-core` once merged) | Same `openapi-typescript` flow → `ts/types/src/generated/core.ts`; `CORE_ENDPOINT_MANIFEST` (4 entries). | Same rule as `ts/client`, plus: verdict arms come from `specs/typespec/govern/main.tsp`'s `Verdict` enum. |
