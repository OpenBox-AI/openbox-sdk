...
/// Generated Rust types and async client for the OpenBox backend API.
/// Surfaced under a module name rather than re-exported flat so types
/// like `Approval` do not collide with anything we hand-write later,
/// such as wire-shape helpers or sync wrappers built on top of this.
pub mod backend {
