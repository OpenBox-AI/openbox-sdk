...
#
# What it does:
#   1. Starts postgres + redis + keycloak via openbox-backend/docker-compose.yml
...
#   - Bootstrap the Keycloak `openbox` realm, `openbox-backend` client, or
#     `openbox-backend-fe` client. Those require admin-UI choices we can't
...
BACKEND_DIR="${BACKEND_DIR:-$HOME/workspace/openbox-repos/openbox-backend}"
...
[[ -d "$BACKEND_DIR" ]] || fail "openbox-backend not found at $BACKEND_DIR (set BACKEND_DIR)"

# 1. docker-compose up -d
if ! docker info >/dev/null 2>&1; then
  fail "Docker daemon isn't running. Start Docker Desktop and re-run."
fi
info "starting postgres + redis + keycloak via openbox-backend/docker-compose.yml"
...
if [[ ! -f "$ENV_FILE" ]]; then
  ...
  sed -i.bak \
    ...
    -e 's|^KEYCLOAK_CLIENT_ID=.*|KEYCLOAK_CLIENT_ID=openbox-backend|' \
    -e 's|^KEYCLOAK_CLIENT_FE_ID=.*|KEYCLOAK_CLIENT_FE_ID=openbox-backend-fe|' \
