...
latest `openbox-backend` main, so most testing does NOT need a local stack.
Use this guide when:
...
- `openbox-backend` cloned at `~/workspace/openbox-repos/openbox-backend`
...
`openbox-backend/docker-compose.yml`, waits for Keycloak to respond, generates
...
```bash
cd ~/workspace/openbox-repos/openbox-backend
yarn start:dev
```
...
- Local **openbox-core** (the Go service at port 8086 for runtime governance
  ...
  `~/workspace/openbox-repos/openbox-core/docker-compose.yml` for that stack.
- Local **openbox-fe** - not needed for CLI testing. Login uses Keycloak's
