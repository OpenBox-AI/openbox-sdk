...
**The check is NOT in the backend source.** Grep the `the-backend-service` repo for `X-Openbox-Client`, `openbox-client`, or `No auth token` and you get zero hits. `src/modules/auth/strategies/jwt.strategy.ts` only calls `ExtractJwt.fromAuthHeaderAsBearerToken()`.
...
Implications:
- **Self-hosted parity**: running `the-backend-service` without an equivalent edge proxy gives you a backend that accepts any request with a valid bearer (no header required). Add the check in your own reverse proxy if you want parity.
...
The CLI, first-party SDKs, and `runtime/mcp` send the header automatically. Raw HTTP clients must:
...
3. **FE bug**: `the-dashboard-fe`'s `lib/auth-options.ts:57` sends `body: JSON.stringify({ refresh_token: token.refreshToken })` - snake_case - but the backend's `RefreshDto.refreshToken` is camelCase → the body is stripped by the whitelist pipe and the handler receives `{}` → 422 (or the handler's `user.sub` path hits the Keycloak 500 first; order depends on request shape).
...
Source references: `the-backend-service/src/modules/auth/auth.controller.ts:81` (wrong-claim bug), `src/modules/auth/dto/auth.dto.ts:69` (RefreshDto.refreshToken camelCase), and `the-dashboard-fe/lib/auth-options.ts:57` (FE sends snake_case).
