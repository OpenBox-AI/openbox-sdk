...
When an event hits `POST /api/v1/governance/evaluate`, openbox-core runs a Temporal workflow with this fixed activity sequence (see `internal/services/governance_workflow.go`):
...
6b. GuardrailsCheckActivity        - calls openbox-guardrails service
