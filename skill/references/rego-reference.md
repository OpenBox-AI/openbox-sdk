...
- [Input fields available to Rego](#input-fields-available-to-rego)
...
OpenBox uses OPA, the Open Policy Agent, with a specific Rego shape.
Policies must return `result` with `decision` and `reason`. Do **not**
use `deny[msg]`. Core reads `result.decision` and `result.reason`. A
`deny[msg]` rule leaves `result` undefined; core falls back to
`ALLOW`, and the policy silently does nothing.
...
Decisions are matched case-insensitively by core. Accepted values:

| Value | Maps to verdict |
|---|---|
| `allow`, `continue` | ALLOW |
| `block`, `stop` | BLOCK |
| `halt` | HALT |
| `require_approval`, `require-approval` | REQUIRE_APPROVAL |
| anything else, such as `DENY`, `REJECT`, `CONSTRAIN` | silently falls through to ALLOW |

Convention is uppercase: `ALLOW`, `BLOCK`, `HALT`, `REQUIRE_APPROVAL`.
Both forms work; uppercase reads better.

**Package name is rewritten server-side.** The backend strips whatever
`package …` line you write and replaces it with
`package org.<sanitized_orgId>.policy_<policyIdNoDashes>`, and core
queries that rewritten path. Write `package org.openbox_ai.anything`;
the name is purely decorative. Put all rules in one package; do not
split across packages.

**`default` assignment** uses `:=`, not `=`, in Rego v1. Older
`default result = {...}` with a single `=` still parses but is
deprecated. Use `:=` everywhere for forward compatibility.

**Workflow events bypass OPA.** `WorkflowStarted`, `WorkflowCompleted`,
and `WorkflowFailed` auto-allow in core, so policies targeting those
event types never run. OPA only evaluates on `ActivityStarted`,
`ActivityCompleted`, and `SignalReceived`.

## Input fields available to Rego

Core populates a rich `input` object. The table below lists what's
guaranteed to be at root; anything not listed stays nested inside
`input.activity_input[0].*`.
...
input.run_id                          # run ID
input.activity_id                     # per-action; only on Activity* events
...
input.trust_tier                      # 1-4. Also exposed as input.risk_tier; same value, duplicated

# Extracted from activity_input[0] to root. These are the only hoisted fields.
...
input.activity_input[0].command       # shell command for ShellExecution
input.activity_input[0].file_path     # file path for FileRead
input.activity_input[0].tool_name     # MCP tool name for MCPToolCall
...
input.status                          # "completed", "failed", "cancelled", or "terminated"
input.error                           # { type, message } on failure
input.duration_ms                     # activity wall-clock

# Spans. Each is span-shaped with attributes.
...
**`activity_input` can be an array or an object.** Core tolerates
both shapes. Policies that hardcode `input.activity_input[0]` will
throw on object-shape inputs. Defensive policies should check the
type or target only the downstream-extracted root fields,
`input.prompt` and `input.messages`.

**Rego v1 compatibility.** If you hit `"undefined identifier: if"` or
similar, your OPA runtime is pre-v1. Add `import rego.v1` at the top
to force v1 semantics: `if`, `in`, and `contains` become keywords
rather than bare identifiers. OpenBox ships a modern OPA, so the
import isn't required there today, but portable policies should
include it.
...
# OR: separate rules with the same result
...
### Combined policy with multiple rules in one file

Only one policy can be active per agent. Combine rules into a single
file:
...
Creating a new policy automatically deactivates any existing one;
core enforces "current version" semantics. To combine multiple rule
sets on one agent, merge them into a single `.rego` file with
separate `result := ...` rules.

### Approval timeout: OPA policies cannot set it

A `REQUIRE_APPROVAL` verdict produced by an OPA policy uses a
server-side default. `CreatePolicyDto` has no `approval_timeout`
field; neither the create form, nor the stored record, nor the Rego
return shape `{decision, reason}` carries one. Do not pass
`--approval-timeout` to `policy create`; the flag does not exist on
that command.

**To control the timeout, use a behavior_rule instead.**
`CreateBehaviorRuleDto` has `approval_timeout: numeric`, required
when `verdict=2`. Behavior rules and OPA policies coexist on the same
agent. Both run during `core evaluate`, and the strictest verdict
wins. If you need a 5-minute window, attach a behavior_rule with
`--verdict 2 --approval-timeout 300` matching the same trigger. Do
not try to express the timeout inside Rego.

**Do not conflate the two paths.** When asked "create an approval
...
`policy update` is a rollback or toggle, not an editor. The
`UpdatePolicyDto` only defines `is_active`, `trust_impact`, and
`trust_threshold`. A `rego_code` in the PUT body is silently dropped
by NestJS's whitelist pipe; the endpoint returns 200 but the stored
rego is unchanged.

The dashboard confirms this: clicking "Edit Policy" calls the create
endpoint, not the update endpoint.

> **Bundle redeploy is conditional.** `policy create` always
> triggers an OPA bundle rebuild and S3 upload. `policy update` only
> redeploys when `is_active` is being set to true. `policy delete`
> does not trigger a rebuild. To change rego code, always use
> `policy create`.

Behavior of `PUT /agent/{id}/policies/{policyId}`:

1. Transaction begins.
2. Sets `is_active=false` and `is_current_version=false` on every
   policy on the agent.
3. On the targeted policy: sets `is_active = body.is_active` and
   `is_current_version = true`, plus any provided `trust_impact` and
   `trust_threshold`.
4. Redeploys if active.

So to "update" a policy, POST a new one with the changed rego. To
"roll back", PUT `{is_active: true}` on the older version's ID. To
"toggle off", PUT `{is_active: false}`.

`is_active` is `@IsBoolean()` with no `@IsOptional()`. Omitting it on
the PUT body returns 422 with `is_active must be a boolean value`.
Always pass it explicitly.

### Input extraction: only `prompt` and `messages` land at root

Core constructs the OPA input by extracting two fields from
`activity_input[0]` to the root level:

- `prompt` → `input.prompt`
- `messages` → `input.messages`

Every other field stays nested under `input.activity_input[0].*`.
For shell commands, file paths, MCP tool inputs, etc., policies
must access them through the array:
...
That's why prompt policies can do `input.prompt` directly but
tool-call policies must go through `activity_input[0]`.
