...
Everything specific to configuring and debugging guardrails - the content-validation layer that runs via the FastAPI `openbox-guardrails` service behind core.
...
The guardrails service (`openbox-guardrails/src/services/guardrails.py:GUARDRAILS_MAP`) maps these numeric IDs to GuardrailsHub validators:
...
`openbox-backend`'s `guardrail.service.ts` defaults `trust_impact` to `NONE` on create when the field is omitted. TrustImpact enum: `none | low | medium | high` (`trust.enum.ts`).
