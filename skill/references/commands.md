...
Clones `OpenBox-AI/openbox-skill` into `~/.claude/skills/openbox` (when `--claude`) or `~/.cursor/skills/openbox` (when `--cursor`). With no flag, installs into both if available.

### `openbox setup mcp [--dir <path>]`

Clones `OpenBox-AI/openbox-mcp`, builds it, and registers the server in Cursor's MCP config (`~/.cursor/mcp.json`). `--dir` overrides the install location (default: `~/workspace/openbox-mcp`).
