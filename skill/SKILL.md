...
> Based on the-core-service `591f66f`, the-backend-service `3d4735d`, the-guardrails-service `73478ad`. Last refreshed against live APIs on 2026-04-23.
...
If the user is on `runtime/mcp` (the MCP server) instead of a direct CLI shell, the MCP server reads its calling client's name from the MCP `initialize` handshake and sets `runtime/mcp/<caller>` automatically - you don't need `OPENBOX_CLIENT_VARIANT` in that path.
