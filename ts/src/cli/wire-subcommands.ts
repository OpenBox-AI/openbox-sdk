...
export const OUTPUT_POST_REGISTRY: Record<string, (data: unknown) => void> = {
   ...
   *  after a rotate. Mirrors highlightRuntimeKey but for `secret`. */
...
function buildBody(opts: Record<string, unknown>, sub: SubcommandSpec): Record<string, unknown> {
  ...
  for (const flag of sub.flags) {
    ...
    // shapes that include the field, such as the createUser body's
    // `roles`, expect an array rather than an absent key.
  ...
  switch (sub.output.kind) {
    ...
    case 'binary':
      ...
      if (typeof renderable === 'string') {
        ...
        // Fallback for objects, such as a wrapped `{data: bytes}`. JSON-dump it.
        output(renderable);
      }
      break;
    case 'table':
