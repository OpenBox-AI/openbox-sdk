...
3. The four emitted verdicts (`allow`, `require_approval`, `block`, `halt`) and the placeholder `constrain` are the spec's `Verdict` enum at `specs/typespec/govern/main.tsp`. New verdict arms get added there first, then in the wrapper's verdict handler.
