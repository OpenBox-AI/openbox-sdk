/**
 * Single source for the tool-input extractor family.
 *
 * These helpers (`filePathFor`, `httpTargetFor`, `httpMethodFor`, the `db*For`
 * family and the `is*McpTool` classifiers) were previously copy-pasted across
 * the claude-code mapper, the anthropic/openai payload builders and the
 * copilotkit pipeline. They drifted (notably openai's `filePathFor` had lost
 * `notebook_path`). They now live here so there is exactly one canonical
 * implementation — edit here, never fork. No drift.
 *
 * Built on the shared trimmed string helpers in `./strings.js`.
 */
import { firstTrimmed as firstString } from './strings.js';
