...
- `runtime/mcp-skunkworks` - MCP server
...
For non-TS clients (Rust, Go, Python): port the env table + token codec from `packages/env/src/environments.json` and `packages/env/src/token-codec.ts`. The Rust port in `the-approver-app/src-tauri/src/env.rs` is the reference.
...
| MCP server (`runtime/mcp-*`) | `OPENBOX_ENV` env var → `production` |
...
Backend's auth guard is presence-only - any value passes. The value is purely a telemetry dimension. Set `clientName` to your app's identifier (`apps/extension`, `the-mobile-app`, `runtime/mcp`, `the-approver-app`, ...).
...
For MCP servers, read `mcp.server.getClientVersion()?.name` after `server.connect()` and feed it into a per-request `X-Openbox-Client: runtime/mcp/<caller>` header. Reference: `runtime/mcp-skunkworks/src/config.ts:setMcpClientName`.
