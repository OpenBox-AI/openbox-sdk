...
describe('setMcpClientName + createApi header', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'openbox-mcp-headers-'));
    tokensPath = join(tmpDir, 'tokens');
  ...
  it('defaults to bare openbox-mcp before initialize', async () => {
    expect(await captureHeader()).toBe('openbox-mcp');
  });

  it('appends the MCP client name once set', async () => {
    setMcpClientName('claude-code');
    expect(await captureHeader()).toBe('openbox-mcp/claude-code');
  });
