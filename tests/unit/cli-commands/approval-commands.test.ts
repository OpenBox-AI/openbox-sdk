...
import { registerApprovalCommands } from '../../../ts/src/cli/commands/approval.js';
...
describe('approval decide', () => {
  it('accepts action=approve', async () => {
    ...
    // CLI calls decideApproval(agentId, eventId, { action }). The body
    // object shape replaced the positional string when the wire schema
    // grew optional fields (reason, etc).
    expect(mockClient.__calls[0].args).toEqual(['agent-1', 'event-1', { action: 'approve' }]);
  });
  ...
  it('accepts action=reject', async () => {
    ...
    expect(mockClient.__calls[0].args[2]).toEqual({ action: 'reject' });
  });
