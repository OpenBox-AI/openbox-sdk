...
describe('OpenBoxClient', () => {
  ...
  describe('construction', () => {
    ...
    it('sends X-Openbox-Client with the configured clientName', async () => {
      const client = createClient({ clientName: 'openbox-extension' });
      fetchMock.mockResolvedValueOnce(mockResponse(200, { data: 'ok' }));

      await client.health();

      const headers = fetchMock.mock.calls[0][1].headers as Record<string, string>;
      expect(headers['X-Openbox-Client']).toBe('openbox-extension');
    });
    ...
    it('appends OPENBOX_CLIENT_VARIANT to the header', async () => {
      ...
      try {
        const client = createClient({ clientName: 'openbox-mcp' });
        fetchMock.mockResolvedValueOnce(mockResponse(200, { data: 'ok' }));

        await client.health();

        const headers = fetchMock.mock.calls[0][1].headers as Record<string, string>;
        expect(headers['X-Openbox-Client']).toBe('openbox-mcp/claude-code');
      } finally {
