    ...
    .action(async () => {
      console.error(
        [
          ...
          '  • the-backend-service passes user.sub as Keycloak realm (should be user.orgId)',
          '    fix: the-maintainer/the-backend-service  fix/auth-refresh-passes-wrong-arg',
          '  • the-dashboard-fe sends { refresh_token } (should be { refreshToken })',
          '    fix: the-maintainer/the-dashboard-fe       fix/auth-refresh-body-camelcase',
          '',
