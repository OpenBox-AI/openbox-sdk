...
const MCP_REPO = 'https://github.com/OpenBox-AI/openbox-mcp.git';
...
export function registerSetupCommands(program: Command) {
  ...
  setup
    ...
    .option('--dir <path>', 'Install directory', join(homedir(), 'workspace', 'openbox-mcp'))
    .action((opts) => {
  ...
  setup
    ...
    .option('--dir <path>', 'MCP install directory', join(homedir(), 'workspace', 'openbox-mcp'))
    .action((opts) => {
